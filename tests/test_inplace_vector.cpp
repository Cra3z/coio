...
TEST_CASE("inplace_vector handles range construction and mutation") {
    ...
    CHECK_EQ(values, coio::inplace_vector<int, 8>{1, 2, 3, 4, 5});

    auto where = values.insert_range(values.cbegin() + 2, std::array{9, 8});
    CHECK_EQ(where, values.begin() + 2);
    CHECK_EQ(values, coio::inplace_vector<int, 8>{1, 2, 9, 8, 3, 4, 5});

    const auto removed = coio::erase_if(values, [](int value) noexcept { return value % 2 == 0; });
    CHECK_EQ(removed, 3);
    CHECK_EQ(values, coio::inplace_vector<int, 8>{1, 9, 3, 5});

    coio::inplace_vector<int, 8> other{7, 11};
    std::ranges::swap(values, other);

    CHECK_EQ(values, coio::inplace_vector<int, 8>{7, 11});
    CHECK_EQ(other, coio::inplace_vector<int, 8>{1, 9, 3, 5});
}
